...
namespace SpellEditor.Sources.DBC
{
    class SpellRuneCost
    {
        ...
        public SpellRuneCost(MainWindow window, SQLite.SQLite SqliteConn)
        {
            main = window;
            Sqlite = SqliteConn;
        ...
        {
			int ID = Int32.Parse(Sqlite.query(string.Format("SELECT `RuneCostID` FROM `{0}` WHERE `ID` = '{1}'", Sqlite.Table, main.selectedID)).Rows[0][0].ToString());
